...
|0000B7 |1|Binary|HDMAEN shadow|Written to `$420C` at `$80:83CD` - but **only** on NMI-tail branch 0. Branch 2 (`$80:83D6`) copies it to `$0EE6` and never touches `$420C`. So injected code that disables HDMA **must** re-enable it itself; assuming the NMI will is wrong and leaves the screen black in any branch-2 state
|0000C0 |1|Unsigned|Viewport Something ?
|0000FA |1|Unsigned|NMI tail dispatch index x 2|`$80:83BD` does `LDX $FA` then `JMP ($83C2,X)`; the table at `$83C2` holds `$83C6` and `$83D6`. Entry 0 refreshes `$2100` from `$00A0` and `$420C` from `$00B7`. Entry 2 force-blanks (`$2100 = $80`), arms an H/V IRQ via `$4207`/`$4209` and draws by raster effect, so it depends on HDMA staying enabled
...
|000EE6 |1|Binary|HDMAEN shadow, branch-2 copy|NMI tail branch 2 writes `$00B7` here at `$80:83EC` and branches on it being zero. Not the same thing as `$00B7`, which is what actually reaches `$420C`
|000EEB |1|Binary|Copier-detection flag 1|`$FF` once writable RAM is found at `$70:1FFF`, the LoROM SRAM window a genuine cartridge does not have. The test is `LDA/INC/STA/CMP $701FFF` at `$80:8746`; the flag is set at `$80:8757` when the read-back matches. Read at `$84:8A39`, where non-zero makes the crest menu **refuse to open**. Cleared by `STZ` at `$A2:AF88`. Declaring SRAM in the header sets it - see `docs/patches.md` change 5
|000EEC |1|Binary|Copier-detection flag 2|The same `$70:1FFF` test, at `$80:E543` inside the damage handler, setting the flag at `$80:E554`. Read at `$82:88F9`, where non-zero branches over the `SBC $0000,Y` at `$82:8904` that **subtracts damage from an enemy** - so shots stop hurting anything. This is why the symptom appears only after Firebrand takes damage
|000EED |1|Binary|Copier-detection flag 3|Read at `$84:8A3B`, the same menu chain as `$0EEB`. Written at `$82:9B3F`. **Not** SRAM-dependent: it stays clear on a build that declares no SRAM, so whatever it tests currently passes. Condition unidentified
|000EEE |1|Binary|Copier-detection flag 4|Read at `$82:88FB`, the same damage chain as `$0EEC`. Set at `$BE:E3CF` when `$80:FFC1` differs from `$40:FFC1` - a ROM-mirror comparison that catches a badly dumped or overdumped ROM rather than added RAM. Unaffected by this project's patches
